  ...
  while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
    ...
    */
    ...
    /* -----------------------------------------------------------------------------------
      ...
      DISABLED: Moving all spindle PWM computation out of stepper ISR preparation to prevent
      port conflicts with direction pins on PORTC when writing to PORTH during motion.
      Spindle state is now set ONLY via spindle_sync() before motion starts.
    */
    
    // Always use the pre-computed PWM value from planner block without recalculation
